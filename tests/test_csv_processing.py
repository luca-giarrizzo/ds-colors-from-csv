...
def test_csv_processing_load():
    ...
    assert palette.getRGBValues() == TEST_RGB_VALUES


def test_csv_processing_set_get_options():
    csvColorProcessor = CSVColorProcessor()
    csvColorProcessor.setOption("labelRow", 3)
    assert csvColorProcessor.getOption("labelRow") == 3


def test_csv_processing_set_option_type_check():
    csvColorProcessor = CSVColorProcessor()
    assert not csvColorProcessor.setOption("labelRow", "3")


def test_csv_processing_get_option_invalid():
    csvColorProcessor = CSVColorProcessor()
    assert not csvColorProcessor.getOption("myOption")

def test_csv_processing_reset_option():
    csvColorProcessor = CSVColorProcessor()
    csvColorProcessor.setOption("labelRow", 3)
    csvColorProcessor.resetOption("labelRow")
    assert csvColorProcessor.getOption("labelRow") == CSVColorProcessor.CSV_OPTIONS_DEFAULTS["labelRow"]

def test_csv_processing_reset_all_options():
    csvColorProcessor = CSVColorProcessor()
    csvColorProcessor.setOption("labelRow", 3)
    csvColorProcessor.setOption("colorSeparator", ";")
    csvColorProcessor.resetAllOptions()
    checkOption1 = csvColorProcessor.getOption("labelRow") == CSVColorProcessor.CSV_OPTIONS_DEFAULTS["labelRow"]
    checkOption2 = csvColorProcessor.getOption("colorSeparator") == CSVColorProcessor.CSV_OPTIONS_DEFAULTS["colorSeparator"]
